...
LiPkg::LiPkg()
    : mTimestamp(0), mSpeed(0), mErrorTimes(0), mFrameReady(false),
      mIsPkgReady(false) {}

double LiPkg::GetSpeed(void) { return mSpeed; }
  ...
  while (start < mDataTmp.size() - 2) {
    ...
    if (crc == pkg->crc8) {
      ...
      if (diff > (double)pkg->speed * POINT_PER_PACK / 4500 * 3 / 2) {
        ...
        uint32_t diff =
            ((uint32_t)pkg->end_angle + 36000 - (uint32_t)pkg->start_angle) %
            36000;
